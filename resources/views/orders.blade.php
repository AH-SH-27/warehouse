                ...
                <div class="border rounded-lg p-4 mb-4">
                    ...
                    <div class="mt-2">
                        ...
                        <ul class="mt-2 space-y-2">
                            @foreach ($order->store->products as $product)
                            <li class="border p-3 rounded-md flex justify-between">
                                <div>
                                    <p class="font-semibold">{{ $product->name }}</p>
                                    <p class="text-sm text-gray-500">{{ $product->quantity }} x ${{ $product->price }}</p>
                                </div>
                                <p class="font-semibold text-gray-700">${{ number_format($product->quantity * $product->price, 2) }}</p>
                            </li>
                            @endforeach
                        </ul>
                    </div>
